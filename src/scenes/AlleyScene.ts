...
export class AlleyScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    setPresenceCallbacks({
      onPlayerJoin: (p) => {
        if (p.pubkey === myPubkey || this.otherPlayers.has(p.pubkey) || this.isLeavingScene) return;
      ...
      onChat: (pk, name, text) => {
          ...
          if (!isMe) {
            ...
            if (o && (action === 'on' || action === 'off')) {
              ...
              if (action === 'on') {
                ...
                if (flavor && !mutedPlayers.has(pk)) this.chatUI.addMessage(name, flavor, P.dpurp, pk);
        ...
        if (isMe) ChatUI.showBubble(this, this.player.x, this.player.y - 94, text, ALLEY_ACCENT);
      ...
      onAvatarUpdate: (pk, avatarStr) => {
        ...
        o.avatar = avatarStr;
        const cfg = deserializeAvatar(avatarStr) || getDefaultAvatar();
        const texKey = `avatar_hub_${pk}`;
        if (this.textures.exists(texKey)) this.textures.remove(texKey);
        this.textures.addCanvas(texKey, renderHubSprite(cfg));
    ...
    const unsubProfile = authStore.subscribe(() => {
      const n = authStore.getState().displayName;
      if (n && n !== this.registry.get('playerName')) {
        this.registry.set('playerName', n);
        this.playerName?.setText(n.slice(0, 14));
        sendNameUpdate(n);
      }
    });
    ...
    this.events.on('shutdown', () => {
      ...
      this.otherPlayers.forEach(o => {
        o.sprite.destroy(); o.nameText.destroy(); o.statusText.destroy();
  ...
  private createPlayer(): void {
    ...
    this.playerName = this.add.text(spawnX, this.playerY - 90, name.slice(0, 14), {
      ...
      color: ALLEY_ACCENT, align: 'center',
  ...
  private addOtherPlayer(pk: string, name: string, px: number, py: number, avatarStr?: string, status?: string): void {
    ...
    if (dying) { this.tweens.killTweensOf([dying.sprite, dying.nameText, dying.statusText]); dying.sprite.destroy(); dying.nameText.destroy(); dying.statusText.destroy(); if (dying.clickZone) dying.clickZone.destroy(); this.dyingSprites.delete(pk); }

    const texKey = `avatar_hub_${pk}`;
    const cfg = avatarStr ? (deserializeAvatar(avatarStr) || getDefaultAvatar()) : getDefaultAvatar();
    if (this.textures.exists(texKey)) this.textures.remove(texKey);
    this.textures.addCanvas(texKey, renderHubSprite(cfg));
    ...
    const nt = this.add.text(px, py - 90, name.slice(0, 14), { fontFamily: '"Courier New", monospace', fontSize: '9px', color: ALLEY_ACCENT, align: 'center', backgroundColor: '#04081088', padding: { x: 3, y: 1 } }).setOrigin(0.5).setDepth(9);
    const ss = (status || '').slice(0, 30);
    const st = this.add.text(px, py - 102, ss, { fontFamily: '"Courier New", monospace', fontSize: '8px', color: P.lpurp, align: 'center' }).setOrigin(0.5).setDepth(9).setAlpha(ss ? 1 : 0);
