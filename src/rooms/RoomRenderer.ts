...
export class RoomRenderer {
  ...
  private drawDoor(x: CanvasRenderingContext2D, W: number, _doorY: number, nc: string, GAME_HEIGHT: number): void {
    ...
    x.fillStyle = '#3a2878'; x.fillRect(doorX - 10, doorY - 10, doorW + 20, doorH + 14);
    x.fillStyle = '#2a1858'; x.fillRect(doorX - 6, doorY - 6, doorW + 12, doorH + 8);
    ...
    x.globalAlpha = 1;
    x.strokeStyle = nc; x.lineWidth = 1; x.globalAlpha = 0.3;
  ...
  private drawFeed(x: CanvasRenderingContext2D, W: number, H: number, nc: string): void { const r = (ax: number, ay: number, aw: number, ah: number, col: string) => { x.fillStyle = col; x.fillRect(ax, ay, aw, ah); }; const FY = 300; r(0, 0, W, H, '#0c0820'); r(0, FY, W, H - FY, '#120a2c'); r(0, FY, W, 2, nc); r(28, 46, W - 56, 246, '#040208'); r(30, 48, W - 60, 242, '#060412'); r(34, 52, W - 68, 234, '#080616'); x.strokeStyle = nc; x.globalAlpha = 0.25; x.strokeRect(30, 48, W - 60, 242); x.globalAlpha = 1; x.strokeStyle = nc; x.globalAlpha = 0.08; x.strokeRect(34, 52, W - 68, 234); x.globalAlpha = 1; r(40, 54, W - 80, 22, '#0a0818'); x.fillStyle = nc; x.font = 'bold 11px monospace'; x.textAlign = 'center'; x.fillText('GLOBAL NOSTR FEED', W / 2, 70); r(40, 78, W - 80, 1, nc); x.globalAlpha = 0.2; r(40, 78, W - 80, 1, nc); x.globalAlpha = 1; x.fillStyle = P.red; x.globalAlpha = 0.5; x.beginPath(); x.arc(60, 64, 3, 0, Math.PI * 2); x.fill(); x.globalAlpha = 1; x.fillStyle = P.red; x.font = 'bold 7px monospace'; x.textAlign = 'left'; x.globalAlpha = 0.6; x.fillText('LIVE', 66, 67); x.globalAlpha = 1; x.fillStyle = P.teal; x.globalAlpha = 0.4; x.font = 'bold 7px monospace'; x.textAlign = 'right'; x.fillText('STREAMING...', W - 56, 67); x.globalAlpha = 1; for (let i = 0; i < 9; i++) { const ey = 84 + i * 22; r(44, ey, W - 88, 18, i % 2 === 0 ? '#0a0818' : '#0c0a20'); } x.fillStyle = nc; x.globalAlpha = 0.025; r(20, FY, W - 40, 35, nc); x.globalAlpha = 1; this.drawDoor(x, W, FY + 50, nc, H); }
