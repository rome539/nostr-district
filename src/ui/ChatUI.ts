...
export class ChatUI {
  ...
  create(placeholder: string, accentColor: string, onCommand: (text: string) => void): HTMLInputElement {
    ...
    // On touch devices the side button groups occupy --nd-ctrl-offset px on each edge;
    // use left/right anchoring instead of centering so the chat fills the gap between them.
    const isMobile = 'ontouchstart' in window;
    const mobileContainerStyle = isMobile
      ? `left:var(--nd-ctrl-offset,60px);right:var(--nd-ctrl-offset,60px);width:auto;max-width:none;`
      : `left:50%;transform:translateX(-50%);width:520px;max-width:92vw;`;
    this.container.style.cssText = `position:fixed;bottom:8px;${mobileContainerStyle}z-index:1000;font-family:'Courier New',monospace;pointer-events:none;`;
