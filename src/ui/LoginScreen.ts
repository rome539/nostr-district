...
export class LoginScreen {
  ...
  private drawFrame(time: number): void {
    ...
    moonHL.addColorStop(0, 'rgba(255,252,230,0.28)');
    moonHL.addColorStop(0.6, 'rgba(255,252,230,0)');
    ctx.fillStyle = moonHL;
    ctx.beginPath(); ctx.arc(mx, my, mr, 0, Math.PI * 2); ctx.fill();
    // Shadow limb (bottom-right)
    const moonShadow = ctx.createRadialGradient(mx + 6, my + 6, 0, mx, my, mr);
    moonShadow.addColorStop(0.5, 'rgba(0,0,20,0)');
    moonShadow.addColorStop(1, 'rgba(0,0,20,0.32)');
    ctx.fillStyle = moonShadow;
    ctx.beginPath(); ctx.arc(mx, my, mr, 0, Math.PI * 2); ctx.fill();
    // Craters
    ctx.fillStyle = 'rgba(0,0,18,0.18)';
