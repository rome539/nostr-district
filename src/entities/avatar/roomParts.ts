...
export function drawRoomHairSidesOnly(x: CanvasRenderingContext2D, hair: string, oY: number): void {
  switch (hair) {
    case 'long': {
      const d = HAIR_DEFS.long;
      if (d.hatRoomKey && imgCache.has(d.hatRoomKey)) { drawHairNative(x, d.hatRoomKey, d.hatRoomX!, oY + d.hatRoomY!, d.flipH); }
      else if (imgCache.has(d.roomKey)) { drawHairNative(x, d.roomKey, d.roomX, oY + d.roomY, d.flipH); }
...
export function drawRoomTop(
  ...
  if (a.top === 'tank') {
    ...
    x.fillStyle = topDark;
    x.fillRect(8, oY + 13, 8, 1);
  } else if (a.top === 'robe' || a.top === 'bitcoinshirt' || a.top === 'ostrichshirt' || a.top === 'camoshirt' || a.top === 'tunic' || a.top === 'skindress' || a.top === 'knightchest' || a.top === 'camolongsleeve' || a.top === 'vjacket' || a.top === 'travelercoat' || a.top === 'twotoneshirt' || a.top === 'twotonelongsleeve' || a.top === 'bannercoat') {
  }
  ...
  const roomTopPngKey = ({
    ...
    twotonelongsleeve: 'top_twotonelongsleeve_room',
    bannercoat:        'top_bannercoat_room',
