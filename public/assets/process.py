"""
Asset processor — run from any directory:
  python3 public/assets/process.py

Auto-discovers every *_orig.png in hats/, accessories/, tops/, bottoms/,
removes the black background, crops to content, and saves <name>.png
in the same folder. Never edit this file to add new items.
"""
...
def remove_black_bg(pixels, threshold=20):
    h = len(pixels)
    w = len(pixels[0])
    is_bg = lambda r, g, b: r <= threshold and g <= threshold and b <= threshold

    # Flood-fill from all edges to find background-connected black pixels only.
    # Interior black pixels (eyes, dark details) are not reachable and stay opaque.
    visited = [[False] * w for _ in range(h)]
    stack = []
    for x in range(w):
        for y in [0, h - 1]:
            if not visited[y][x] and is_bg(*pixels[y][x][:3]):
    ...
    while stack:
        ...
        for dy, dx in ((-1,0),(1,0),(0,-1),(0,1)):
            ny, nx = y + dy, x + dx
            if 0 <= ny < h and 0 <= nx < w and not visited[ny][nx] and is_bg(*pixels[ny][nx][:3]):
    ...
    for y, row in enumerate(pixels):
        ...
        for x, (r, g, b, a) in enumerate(row):
            new_row.append((r, g, b, 0) if visited[y][x] else (r, g, b, a))
        out.append(new_row)
    return out
    ...
    for y, row in enumerate(pixels):
        for x, (_,_,_,a) in enumerate(row):
            if a > 10:
                ...
                if y > max_y: max_y = y
    if max_x < min_x:
        print('  WARNING: no visible pixels found, skipping crop')
        return pixels, w, h
...
slots = ['hats', 'accessories', 'tops', 'bottoms']
found = 0

for slot in slots:
    pattern = os.path.join(base, slot, '*_orig.png')
    for src in sorted(glob.glob(pattern)):
        name = os.path.basename(src).replace('_orig.png', '')
        dst  = os.path.join(base, slot, f'{name}.png')
        print(f'[{slot}] {os.path.basename(src)} → {name}.png')
        pixels, w, h = read_png(src)
        pixels = remove_black_bg(pixels)
        pixels, w, h = crop_to_content(pixels, h, w)
        save_png(dst, pixels, w, h)
        print(f'  Saved {name}.png ({w}x{h})')
        found += 1

if found == 0:
    print('No *_orig.png files found. Drop originals into hats/, accessories/, tops/, or bottoms/ with an _orig suffix.')
else:
